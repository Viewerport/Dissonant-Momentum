...
window.addEventListener("load", async function(){
  ...
  try{
    ...
    function getFrequencies(){
      ...
      if (centDeviation > 0){
        intonation.innerHTML = "SHARP BY " + Math.abs(centDeviation) + " CENTS";
      }else
      if (centDeviation < 0){
        intonation.innerHTML = "FLAT BY " + Math.abs(centDeviation) + " CENTS";
      }else{
        intonation.innerHTML = "AMAZING AND COOL BECAUSE YOU ARE PERFECTLY IN TUNE!!!!!!";
      }
